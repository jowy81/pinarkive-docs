...
You can target a cluster when uploading or pinning via the API (form/body field `cl`). Your allowed clusters are returned by [GET /users/me/clusters](/api/get-clusters).
...
See [Gateways](/concepts/gateways) and [POST /files](/api/post-files) / [POST /pin](/api/post-pin) for how to set `cl`.
